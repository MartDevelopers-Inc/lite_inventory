...
INNER JOIN products p ON p.product_id = s.sale_product_id WHERE
DATE(s.sale_datetime)=CURDATE() AND p.product_store_id = '{$view}'";
...
/* Out Of Stock */
$query = "SELECT COUNT(*)  FROM `products` WHERE product_quantity <= 1 AND product_store_id = '{$view}'";
$stmt = $mysqli->prepare($query);
$stmt->execute();
