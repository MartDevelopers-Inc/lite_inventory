<script>
    ...
    WHERE DATE(s.sale_datetime) = CURDATE() AND s.sale_payment_method = 'Cash' AND p.product_store_id = '{$view}'";
    ...
    INNER JOIN products p ON p.product_id = sale_product_id
    INNER JOIN users us ON us.user_id = s.sale_user_id
    WHERE DATE(s.sale_datetime) = CURDATE() AND s.sale_payment_method = 'Mobile Payment' AND p.product_store_id = '{$view}'";
    $stmt = $mysqli->prepare($ret);
    $stmt->execute();
    $res = $stmt->get_result();
    ...
    INNER JOIN products p ON p.product_id = sale_product_id
    INNER JOIN users us ON us.user_id = s.sale_user_id
    WHERE DATE(s.sale_datetime) = CURDATE() AND s.sale_payment_method = 'Credit' AND p.product_store_id = '{$view}'";
    $stmt = $mysqli->prepare($ret);
    $stmt->execute();
    $res = $stmt->get_result();
    ...
    INNER JOIN users us ON us.user_id = s.sale_user_id
    WHERE DATE(s.sale_datetime) = CURDATE() AND s.sale_payment_method = 'Credit' AND s.sale_payment_status = 'Unpaid' 
    AND s.sale_credit_expected_date < CURDATE() AND p.product_store_id = '{$view}'";
    $stmt = $mysqli->prepare($ret);
    $stmt->execute();
    $res = $stmt->get_result();
    if ($credit = $res->fetch_object()) {
