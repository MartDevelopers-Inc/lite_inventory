...
if ($log_type == 'All Logs') {
    ...
    /* Excel File Name */
    $fileName = $log_type . ".xls";

    /* Excel Column Name */
    $fields = array('User Details', 'IP Address', 'Date Created', 'Log Details');
    ...
    $query = $mysqli->query("SELECT * FROM system_logs l INNER JOIN users u
    ...
    if ($query->num_rows > 0) {
        ...
        while ($row = $query->fetch_assoc()) {
            ...
            $lineData = array($row['user_name'] . ' ' . $row['user_email'], $row['log_ip_address'], $log_date, $row['log_details']);
    ...
    $query = $mysqli->query("SELECT * FROM system_logs l INNER JOIN users u
    ON u.user_id = l.log_user_id WHERE l.log_created_at BETWEEN '{$start}' AND '{$end}' 
    AND l.log_type = '{$log_type}' ORDER BY l.log_created_at DESC");
    if ($query->num_rows > 0) {
        /* Load All Fetched Rows */
        while ($row = $query->fetch_assoc()) {
            /* Sanitize Log Date */
